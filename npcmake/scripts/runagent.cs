...
namespace JKress.AITrainer
{
    public class runagent : Agent
    {
        ...
        [Range(0.1f, 10)][SerializeField] float m_TargetWalkingSpeed = 6;
        float m_minWalkingSpeed = 0.1f;
        float m_maxWalkingSpeed = 10;
        ...
        void FixedUpdate()
        {
            ...
            // Reward for maintaining body position
            var bodyPositionReward = 1.0f - Mathf.Abs(head.position.y - hips.position.y);
            AddReward(bodyPositionReward);

            if (earlyTraining)
            {
                // Forces movement towards target (penalize stationary swinging)
